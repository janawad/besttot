<?php
    ...
    function Addblog($request){
		...
		$query = "INSERT INTO blog(date_created,blog_title,blog_image,blog_image_details) VALUES (NOW(),'".Generic::preventSqlInjection($blog_title)."','$blog_image','$blog_image_details')";
    ...
    */
    function Updateblog($request){
		...
		$query = "update blog set date_updated = NOW(),blog_title = '".Generic::preventSqlInjection($blog_title)."' ".$cond1." ".$cond2." where id = $blog_id";
		$result = mysql_query($query);
		if($result){
			$target_path = "uploads/blog/";
			move_uploaded_file($_FILES['blog_image']["tmp_name"],$target_path.$blog_image);
